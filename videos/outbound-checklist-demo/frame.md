...
description: >
  ...
  graph-paper grid, DM Sans serif 400 + DM Sans + DM Mono, the top/bottom cobalt
...
colors:
  ...
  ink-faint: "rgba(56, 189, 248, 0.18)"

typography:
  # — reading ramp —
  body:        { fontFamily: "DM Sans", cqw: 0.83, weight: 400, lineHeight: 1.5 }
  body-lede:   { fontFamily: "DM Sans", cqw: 0.95, weight: 400, lineHeight: 1.5 }
  micro:       { fontFamily: "DM Sans", px: 13, weight: 600, tracking: "0.16em", upper: true }
  micro-strong:{ fontFamily: "DM Sans", px: 16, weight: 600, tracking: "0.18em", upper: true }
  ...
  # — display / hero ramp (DM Sans 400, negative tracking) —
  table-name:  { fontFamily: "DM Sans", cqw: 1.5, weight: 400, lineHeight: 1.15 }
  row-headline:{ fontFamily: "DM Sans", cqw: 2.1, weight: 400, lineHeight: 1.05 }
  ed-callout:  { fontFamily: "DM Sans", cqw: 2.6, weight: 400, lineHeight: 1.1, italic: true }
  headline:    { fontFamily: "DM Sans", cqw: 4.6, weight: 400, lineHeight: 0.95 }
  headline-index:{ fontFamily: "DM Sans", cqw: 5.0, weight: 400, lineHeight: 0.95 }
  display-quote:{ fontFamily: "DM Sans", cqw: 5.7, weight: 400, lineHeight: 1.05, tracking: "-0.005em" }
  display-chapter:{ fontFamily: "DM Sans", cqw: 6.8, weight: 400, lineHeight: 1.0, tracking: "-0.005em" }
  display-closing:{ fontFamily: "DM Sans", cqw: 9.4, weight: 400, lineHeight: 0.96, tracking: "-0.005em" }
  display-hero:{ fontFamily: "DM Sans", cqw: 10.4, weight: 400, lineHeight: 0.9, tracking: "-0.008em" }
  vbig-numeral:{ fontFamily: "DM Sans", cqw: 12.5, weight: 400, lineHeight: 0.9, tracking: "-0.015em" }
...
components:
  ...
  ledger-row:
    layout: "grid: mono num · DM Sans name · Hanken desc · mono delta"
...
- **Fonts** — already set to **DM Sans** (display) / **DM Mono** (body); ignore any preset font name lingering in prose.
...
The voice is a three-face conversation: **DM Sans** serif at weight 400 (size, not weight,
makes hierarchy) carries every display and headline; **DM Sans** carries body and
...
- **DM Sans 400** for all display (negative tracking); **Hanken 600** uppercase 0.16em labels; **DM Mono** chrome.
...
- **Squint** — one DM Sans element dominates at **3–6× its nearest neighbor** (`display-hero`/`vbig-numeral`); hierarchy is size, never weight.
...
Hanken→DM Sans, from a mono delta arrow, or from opacity, never from color.

## Typography

Two ramps. The **reading ramp** (Hanken body 0.83cqw, mono chrome in px) carries copy + labels;
the **display/hero ramp** (DM Sans `headline` 4.6cqw → `vbig-numeral` 12.5cqw) carries every
...
- **DM Sans at 400 only**, in cobalt, with negative tracking (hero −0.008em, numeral −0.015em). **Hanken labels uppercase, 0.16–0.18em.** **DM Mono 0.04–0.08em.** No bold serif.
...
**Ground** paper + grid, hairlines. **Composes** pixel-glitch (right, ~26cqw), qr-block (top-right), display-hero, ed-callout. **Focal** a 1–2 line `display-hero` DM Sans headline in cobalt, left-anchored, with an italic `ed-callout` subtitle in `{colors.ink-soft}`. **Chrome** Hanken kicker; mono meta top + page number. **Accent** none (cobalt is the only ink). **Silence** ~45% paper. **Fixed** DM Sans 400, glitch + QR present, grid + hairlines. **Free** title, glitch step pattern, QR placement. **Density** sparse.

### 2 · Index Ledger (catalog · move: dense matrix · left — the dense frame)

**Ground** paper + grid, hairlines, `pad-top`/`pad-bottom`. **Composes** topbar-rule, ledger-rows. **Focal** a topbar (`headline-index` + mono lab-tag, 1.5px rule) over 4–6 ledger rows (mono num · DM Sans name · Hanken desc), ink-faint dividers. **Chrome** page number. **Accent** mono delta arrows. **Silence** tight — the density exception (the grid wants filling). **Fixed** 1.5px topbar rule, 1px ink-faint dividers, DM Sans names. **Free** rows, lab-tag, deltas. **Density** dense-exception.

### 3 · Chapter Opener (section · move: scale · sparse · left)

**Ground** paper + grid, hairlines. **Composes** pixel-glitch (left, ~14cqw, low opacity), mono index, display-chapter, body-lede. **Focal** a `display-chapter` DM Sans title, with a small mono `CHAPTER NN` index above and an optional Hanken lede ≤42cqw. **Accent** none. **Silence** ~60% — let the grid breathe. **Fixed** DM Sans 400, glitch low-opacity, grid showing. **Free** title, lede, glitch side. **Density** sparse.
...
**Ground** paper + grid, hairlines. **Composes** display-quote (or display-manifesto), attribution rule, optional compact glitch. **Focal** a 2–3 line DM Sans pull in cobalt, with a Hanken kicker above and a 1px cobalt attribution rule + mono byline beneath. **Accent** none. **Silence** ~55% — deliberately open. **Fixed** DM Sans 400, attribution rule, grid showing. **Free** quote, byline. **Density** sparse.

### 6 · Colophon (closer · move: right-aligned close · sparse)

**Ground** paper + grid, hairlines. **Composes** pixel-glitch (left edge, mirroring the cover), display-closing, mono credit columns. **Focal** a right-aligned `display-closing` DM Sans title with a Hanken kicker above. **Chrome** 3–4 column mono credit grid at the foot; page number. **Accent** none. **Silence** ~50%. **Fixed** glitch on left, DM Sans 400. **Free** closing line, credits. **Density** sparse.
...
- Set **DM Sans at 400 in cobalt**; make hierarchy with size, not weight; negative-track display.
...
- Don't bold DM Sans, round any corner, or add a drop shadow (QR outset is an anti-shadow only).
...
- **Squint** — one DM Sans element dominates at 3–5× its neighbor.
...
- **Type** — DM Sans 400 negative-tracked, fit-to-measure; Hanken labels 0.16em; ≥1.4cqw floor.
...
- **Three Google Fonts** (DM Sans, DM Sans, DM Mono); CJK pairing (Noto Serif SC 700/400) carries over from the source.
