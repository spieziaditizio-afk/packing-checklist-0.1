...
The favicon (`<link rel="icon">` in `<head>`) is the app icon embedded inline as a base64 PNG data URI — not a file reference — so the single file stays fully portable (opens correctly even copied alone to another machine). `scanning.png` (repo root) is the source image kept for reference/future re-encoding; editing the icon means re-encoding that PNG to base64 and replacing the data URI, not just swapping a linked file. The same `scanning.png` data is duplicated into the topbar's `.app-icon` image (badge to the left of the Arrow wordmark) — a separate embed, not a shared reference.

The Arrow wordmark is embedded the same way — a base64 PNG data URI duplicated into every `<img>` that shows it (topbar `.logo-icon`, welcome splash `.welcome-logo`, print report `.pr-header-logo`; each tagged `.brand-logo` except the print one, see below) — no shared JS constant, so each `<img src>` is self-sufficient without load-order dependencies. Source artwork is solid black on transparent; `.brand-logo{filter:invert(1)}` turns it white for the dark (default) theme, and `html[data-theme="light"] .brand-logo{filter:none}` drops the invert back to black for light theme. The print report's `.pr-header-logo` is NOT tagged `.brand-logo` — the report's navy header background (`#0a1f36`) is fixed regardless of the app's own theme toggle, so it gets its own unconditional `filter:invert(1)` instead of the theme-switched one. `arrow-logo-transparent.png` (repo root) is the cropped/transparent source kept for re-encoding; re-cropping should trim to the visible glyph's bounding box on the *alpha* channel (or, if working from a flattened/checkerboarded export like the original `image.png`, from a luminance threshold) — a naive RGB-only bbox can pick up faint non-transparent padding and crop wrong.

The Audit PN button's icon (`#audit-btn .audit-icon`) is a small inline `<svg>` of three rounded boxes, not an emoji or image — `fill/stroke:currentColor` so it follows the button's own theme-aware text color automatically. `@keyframes audit-box-pulse` staggers each `<rect>` with `animation-delay` (0s/.3s/.6s) so they light up one after another on a loop, evoking "pick one of several boxes at random to check."

Four more base64-embedded artwork icons follow the same self-sufficient-`<img>` rule: the Order History topbar button (`.btn-icon-img`, source `order history icon.png`) and the `Pallet Type` / `Weight (kg)` / `Height (cm)` field labels (`.lbl-icon`, sources `icons8-pallet-100.png`, `box weight.png` and `measuring tape icon.jpg`). Two gotchas if you re-encode them. The tape is a **JPG on solid white** — it has to be keyed to transparency before embedding (alpha from a per-pixel `min(r,g,b)` threshold with a soft 230–250 ramp) or it shows as a white block on the dark theme. And `.lbl-icon` is sized `height:17px;width:auto` on purpose: the tape and pallet artwork are both wider than tall, so constraining both axes (or square-padding the PNG) shrinks them to an unreadable smudge next to the square box-weight icon.
...
- **Test multi-session**: append `#s=<anything>` to the URL, or click 🆕 "New Order" (topbar) / "+ New Order" (Order History dropdown) — both flush-save the current session and jump to a fresh blank one in the same tab. Each session is independent; opening two browser windows on the same file with different hashes works side-by-side.
...
`updatePalletProgress` is deliberately **not** a sixth one: the Box Progress bar (full-width row under the pallet meta row, live "verified / target" while the operator scans) is *handed* `pickTotal` and `boxTotal` by `updatePalletSummary` rather than walking the DOM itself, so it cannot drift out of sync with the validation strip directly below it. Keep it that way — if you need a new number on the bar, pass it in as an argument. All of the bar's colour lives in a single state class on the `.pallet-progress` wrapper (`p-idle` → `p-low` → `p-mid` → `p-high` → `p-done` → `p-over`), so the JS only ever swaps one className; the shimmer sweep is switched off in the terminal states, and the completion glow (`.pprog-pulse`) fires on the rising edge into `p-done` only, guarded by `_lastProgressDone[pid] === false` so a state restore never triggers it.
